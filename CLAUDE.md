...
- **UI** (`ui/`): 100% Compose screens. Each feature folder has a Screen + ViewModel. State via `StateFlow`. **Compose stability:** State data classes use `@Immutable` (all-immutable fields like `ResultState`) or `@Stable` (sealed hierarchies, classes with mutable fields like `Bitmap` — e.g. `CameraState`, `DownloadState`). Maintain these annotations when adding new state classes. One-shot events (navigation, snackbar) use `Channel<Event>(Channel.BUFFERED)` + `receiveAsFlow()`, NOT `StateFlow<Event?>` + `eventConsumed()`. Camera screen is decomposed into extracted composables: `CameraPreview`, `FrameOverlay`, `CameraPermissionContent`, `AdjustingContent` (gesture handler), each in its own file. Crop geometry lives in `ImageCropper` object (`ui/camera/`), not in the ViewModel. Navigation graph in `ui/navigation/AppNavGraph.kt`; `MainActivity` is a thin shell. **UI dimensions** live in `ui/theme/Dimens.kt` — semantic constants for component sizes, corner radii, font sizes. Standard Material spacing (4/8/12/16/24/32.dp) stays inline. Use `Dimens.*` for non-standard values (button sizes, offsets, custom spacing).
