...
class ScanRepositoryImpl @javax.inject.Inject constructor(private val scanDao: ScanDao) :
  ScanRepository {
  ...
  override suspend fun updateUserFields(
    ...
    val existing = scanDao.getById(scanId)
      ?: throw IllegalStateException("Scan $scanId not found")
    val newStatus =
      if (existing.status == ScanStatus.COMPLETED || existing.status == ScanStatus.EDITED) {
        ScanStatus.EDITED
      } else {
